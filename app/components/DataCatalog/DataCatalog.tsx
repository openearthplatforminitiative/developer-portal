...
export const DataCatalogContent = () => {
  ...
  return (
    <div className="w-full flex flex-col gap-4 mt-16">
      <DataCatalogFilters />
      <div className={`max-h-[700px] transition-all w-full flex ${showMap ? 'gap-10' : 'gap-0'} justify-stretch`}>
        {lg ? (
          <div className={`relative transition-all max-w-2/3 ${showMap ? 'w-full h-[700px]' : 'w-0'}`}>
            <DataCatalogMap />
            <div className="absolute -right-5 top-0 h-full flex justify-center items-center pointer-events-none">
              <Tooltip title="Toggle map">
                <button className="cursor-pointer size-10 rounded-full bg-secondary-90 hover:bg-neutral-90 shadow pointer-events-auto" onClick={() => setShowMap(!showMap)}>{showMap ? <ChevronLeft /> : <ChevronRight />}</button>
              </Tooltip>
            </div>
          </div>
        ) : (
