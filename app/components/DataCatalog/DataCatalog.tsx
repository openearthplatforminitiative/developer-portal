"use client"

import { useState } from "react"
import { DataCatalogList } from "./DataCatalogList"
import { DataCatalogMap } from "./DataCatalogMap"
import { DataCatalogMapAreas } from "./DataCatalogMapAreas"
import { DataCatalogProvider } from "./DataCatalogProvider"
import { DataCatalogSearch } from "./DataCatalogSearch"
import { ChevronLeft, ChevronRight } from "@mui/icons-material"

export const DataCatalog = () => {
  const [showMap, setShowMap] = useState(true)

  return (
    <DataCatalogProvider>
      <div className="w-full flex flex-col gap-5 mt-16">
        <DataCatalogMapAreas />
        <div className="max-h-[700px] w-full flex gap-10 justify-stretch">
          <div className={`relative gap-6 transition-all ${showMap ? 'w-full' : 'w-0'}`}>
            <DataCatalogMap />
            <div className="absolute -right-5 top-0 h-full flex justify-center items-center">
              <button className="size-10 rounded-full bg-neutral-95 hover:bg-neutral-90 shadow" onClick={() => setShowMap(!showMap)}>{showMap ? <ChevronLeft /> : <ChevronRight />}</button>
            </div>
          </div>
          <div className="gap-5 flex flex-col overflow-y-scroll min-w-96 grow">
            <DataCatalogSearch />
            <DataCatalogList />
          </div>
        </div>
      </div>
    </DataCatalogProvider>
  )
}