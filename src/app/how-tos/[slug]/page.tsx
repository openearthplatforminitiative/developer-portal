...
import { fetchLatestResourceTutorials, fetchRelevantResourceTutorialsForTutorial, fetchResourceTutorialBySlug, ResourceTutorial } from "@/sanity/api"
import { ResourceSummary } from "@/types/resource"
import { Typography } from "@mui/material"
import Link from "next/link"
import { notFound } from "next/navigation"

export default async function HowTosPage({ params }: {
...
async function RelevantTutorials({ tutorial }: { tutorial: ResourceTutorial }) {
  ...
  const relevantResourceTitles = tutorial.relevantResources.resources.map((resource) => resource.title)
  const tutorials = await fetchRelevantResourceTutorialsForTutorial(relevantResourceTitles, tutorial.slug.current)
...
async function RelevantResources({ tutorial }: { tutorial: ResourceTutorial }) {
  const directResourcesIds = tutorial.relevantResources.resources.map((resource) => resource.id)
  const excludedResourceTypes = tutorial.relevantResources.excludedResourceTypes || []
  const includeResourceChildren = tutorial.relevantResources.includeResourceChildren || false
  const includeResourceParents = tutorial.relevantResources.includeResourceParents || false
  const directResources = directResourcesIds.map((id) => fetchResource(id).then((resource) => {
    if (!resource) {
      return null
    }
    return resource
