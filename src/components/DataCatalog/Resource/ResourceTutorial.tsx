...
export const Tutorial = ({ tutorial }: ResourceTutorialProps) => {
  ...
  return (
    <Link key={tutorial._id} href={`${params.resourceId}/tutorials/${tutorial.slug.current}`}>
      <div className="group relative w-full bg-card rounded-xl overflow-hidden cursor-pointer">
        ...
        <div className="flex flex-col justify-between items-end h-full p-6 gap-2 absolute w-full bottom-0">
          <ArrowForward fontSize="inherit" className="text-4xl transform transition-transform duration-300 group-hover:translate-x-2 text-neutral-100" />
          <Typography variant="h4" className="text-4xl font-medium text-neutral-100 align-bottom">
            {tutorial.title}
          </Typography>
        </div>
      </div>
