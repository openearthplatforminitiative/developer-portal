...
const hasLinkedResources = (resource: Resource) => {
  return (resource?.parents?.length ?? 0) > 0 || (resource?.children?.length ?? 0) > 0
}
      ...
      <Timeline sx={{
        ...
        {test.map((item) => {
          if (item.isAvailable(resource)) {
            return (
              <TimelineItem color={activeSection === item.id ? "green" : ""} className="cursor-pointer" key={item.id} onClick={() => handleClick(item.id)}>
                <TimelineSeparator>
                  <TimelineDot />
